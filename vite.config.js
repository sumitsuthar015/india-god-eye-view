...
function celestrakProxy() {
  ...
  async function fetchUpstream(group) {
    ...
    const res = await fetch(url.toString(), {
      ...
      headers: { 'User-Agent': 'india-god-eye-view-celestrak-proxy/1.0 (+https://github.com/sumitsuthar015/india-god-eye-view)' },
    });
...
function fetchRegionalPlace(point) {
  const task = _nominatimQueue.then(async () => {
    ...
    const payload = await fetchRegionalJson(`https://nominatim.openstreetmap.org/reverse?${params}`, {
      headers: {
        'User-Agent': 'IndiaGodEyeView/0.1 (+https://github.com/sumitsuthar015/india-god-eye-view)',
        Referer: 'https://github.com/sumitsuthar015/india-god-eye-view',
      },
    });
